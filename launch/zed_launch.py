...
def launch_setup(context, params, *args, **kwargs):
    ...
    xacro_command = []
    custom_baseline = params["custom_baseline"]
    ...
    xacro_command.append(f"{custom_baseline}")   
...
def generate_launch_description():
    ...
    connected_devices = []
    output = sp.getoutput('ZED_Explorer -a').split("\n")
    for entry in output:
        if "S/N" in entry:
            connected_devices.append(entry.replace(' ', '').split(":")[-1])
    ...
    config_files = [f for f in config_list if f.startswith("zed_")]

    # print(config_files)
    #### Reading Config Files ####
    node_list = [SetEnvironmentVariable(name='RCUTILS_COLORIZED_OUTPUT', value='1')]
    for file_name in config_files:
        ...
        serial = data["general"]["serial_number"]
        ...
        if serial_number in connected_devices:
            node_list += [
                DeclareLaunchArgument(
                'camera_name',
                default_value=TextSubstitution(text=data["general"]["camera_name"]),
                description='The name of the camera. It can be different from the camera model and it will be used as node `namespace`.'),
            DeclareLaunchArgument(
                'camera_model',
                default_value = data["general"]["camera_model"],
                description='[REQUIRED] The model of the camera. Using a wrong camera model can disable camera features.',
                choices=['zed', 'zedm', 'zed2', 'zed2i', 'zedx', 'zedxm', 'virtual']),
            DeclareLaunchArgument(
                'node_name',
                default_value=f'Camera_{index}',
            ...
            DeclareLaunchArgument(
                'publish_tf',
                default_value= "true" if data["pos_tracking"]["publish_tf"] else "false",
                description='Enable publication of the `odom -> camera_link` TF.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'publish_map_tf',
                default_value="true" if data["pos_tracking"]["publish_map_tf"] else "false",
                description='Enable publication of the `map -> odom` TF. Note: Ignored if `publish_tf` is False.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'publish_imu_tf',
                default_value="true" if data["sensors"]["publish_imu_tf"] else "false",
            ...
            DeclareLaunchArgument(
                'svo_path',
                default_value=TextSubstitution(text=data["svo"]["svo_path"]),
                description='Path to an input SVO file.'),
            DeclareLaunchArgument(
                'enable_gnss',
                default_value="true" if data["gnss_fusion"]["enable_gnss"] else "false",
            ...
            DeclareLaunchArgument(
                'sim_mode',
                default_value="true" if data["simulation"]["sim_enabled"] else "false",
                description='Enable simulation mode. Set `sim_address` and `sim_port` to configure the simulator input.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'sim_address',
                default_value=data["simulation"]["sim_address"],
            ...
            DeclareLaunchArgument(
                'sim_port',
                default_value=str(data["simulation"]["sim_port"]),
