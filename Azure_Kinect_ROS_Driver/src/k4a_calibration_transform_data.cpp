...
void K4ACalibrationTransformData::initialize(const k4a::device& device, const k4a_depth_mode_t depth_mode,
                                             const k4a_color_resolution_t resolution, const K4AROSDeviceParams& params, std::array<std::string, 5> topic_names)
{
  k4a_calibration_ = device.get_calibration(depth_mode, resolution);
  tf_prefix_ = topic_names[0];
  camera_base_frame_ = topic_names[1];
  std::string rgb_camera_frame_ = topic_names[2];
  std::string depth_camera_frame_ = topic_names[3];
  std::string imu_frame_ = topic_names[4];
  initialize(params);
}
