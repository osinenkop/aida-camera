    ...
    imu_stream_end_of_file_(false)
{
  ...
  this->declare_parameter("device_id", rclcpp::ParameterValue("0"));
  index = this->get_parameter("device_id").as_string();
  index.pop_back();
  // // Declare depth topics
  static const std::string depth_raw_topic = "Camera_" + index + "/depth/image_raw";
  static const std::string depth_rect_topic = "Camera_" + index + "/depth_to_rgb/image_raw";
  static const std::string compressed_format = "Camera_" + index + "/compressed/format";
  static const std::string compressed_png_level = "Camera_" + index + "/compressed/png_level";
...
#undef LIST_ENTRY
  ...
  if (params_.color_format == "jpeg")
  {
    ...
    rgb_jpeg_publisher_ = this->create_publisher<CompressedImage>("Camera_" + index + "/rgb/image_raw/compressed", 1);
  }
  else if (params_.color_format == "bgra")
  {
    rgb_raw_publisher_ = image_transport_->advertise("Camera_" + index + "/rgb/image_raw", 10, true);
  }
  rgb_raw_camerainfo_publisher_ = this->create_publisher<CameraInfo>("Camera_" + index + "/rgb/camera_info", 1);

  depth_raw_publisher_ = image_transport_->advertise("Camera_" + index + "/depth/image_raw", 10, true);
  depth_raw_camerainfo_publisher_ = this->create_publisher<CameraInfo>("Camera_" + index + "/depth/camera_info", 1);

  depth_raw_publisher_ = image_transport_->advertise(depth_raw_topic, 10, true);
  depth_raw_camerainfo_publisher_ = this->create_publisher<CameraInfo>("Camera_" + index + "/depth/camera_info", 1);

  depth_rect_publisher_ = image_transport_->advertise(depth_rect_topic, 10, true);
  depth_rect_camerainfo_publisher_ = this->create_publisher<CameraInfo>("Camera_" + index + "/depth_to_rgb/camera_info", 1);

  rgb_rect_publisher_ = image_transport_->advertise("Camera_" + index + "/rgb_to_depth/image_raw", 10, true);
  rgb_rect_camerainfo_publisher_ = this->create_publisher<CameraInfo>("Camera_" + index + "/rgb_to_depth/camera_info", 1);

  ir_raw_publisher_ = image_transport_->advertise("Camera_" + index + "/ir/image_raw", 10, true);
  ir_raw_camerainfo_publisher_ = this->create_publisher<CameraInfo>("Camera_" + index + "/ir/camera_info", 1);

  imu_orientation_publisher_ = this->create_publisher<Imu>("Camera_" + index + "/imu", 200);

  if (params_.point_cloud || params_.rgb_point_cloud) {
    pointcloud_publisher_ = this->create_publisher<PointCloud2>("Camera_" + index + "/points2", 10);
  }

#if defined(K4A_BODY_TRACKING)
  if (params_.body_tracking_enabled) {
    body_marker_publisher_ = this->create_publisher<MarkerArray>("Camera_" + index + "/body_tracking_data", 1);

    body_index_map_publisher_ = image_transport::create_publisher(this,"Camera_" + index + "/body_index_map/image_raw");
  }
#endif
}
...
k4a_result_t K4AROSDevice::startCameras()
{
  ...
  if (k4a_device_)
  {
    ...
    calibration_data_.initialize(k4a_device_, k4a_configuration.depth_mode, k4a_configuration.color_resolution,
                                 params_, {"", "Camera_" + index + "/camera_base", 
                                 "Camera_" + index + "/rgb_camera_link", 
                                 "Camera_" + index + "/depth_camera_link",
                                 "Camera_" + index + "/imu_link"});
  }
...
void K4AROSDevice::framePublisherThread()
{
  ...
  while (running_ && rclcpp::ok())
  {
    ...
    if (params_.depth_enabled)
    {
      ...
      if ((this->count_subscribers("Camera_" + index + "/ir/image_raw") > 0 || this->count_subscribers("Camera_" + index + "/ir/camera_info") > 0) &&
      ...
      if (calibration_data_.k4a_calibration_.depth_mode != K4A_DEPTH_MODE_PASSIVE_IR)
      {
        ...
        // frame.
          ...
          if ((this->count_subscribers("Camera_" + index + "/depth/image_raw") > 0 || this->count_subscribers("Camera_" + index + "/depth/camera_info") > 0) &&
        ...
        // depth frame.
          ...
          if (params_.color_enabled &&
             (this->count_subscribers("Camera_" + index + "/depth_to_rgb/image_raw") > 0 ||
              this->count_subscribers("Camera_" + index + "/depth_to_rgb/camera_info") > 0) &&
             (k4a_device_ || capture.get_depth_image() != nullptr))
        {
        ...
        if (params_.body_tracking_enabled && k4abt_tracker_queue_size_ < 3 &&
            (this->count_subscribers("Camera_" + index + "/body_tracking_data") > 0 || this->count_subscribers("Camera_" + index + "/body_index_map/image_raw") > 0))
        {
    ...
    if (params_.color_enabled)
    {
      ...
      if (params_.color_format == "jpeg")
      {
        if ((this->count_subscribers("Camera_" + index + "/rgb/image_raw/compressed") > 0 || this->count_subscribers("Camera_" + index + "/rgb/camera_info") > 0) &&
      ...
      {
        if ((this->count_subscribers("Camera_" + index + "/rgb/image_raw") > 0 || this->count_subscribers("Camera_" + index + "/rgb/camera_info") > 0) &&
            (k4a_device_ || capture.get_color_image() != nullptr))
        {
        ...
        if (params_.depth_enabled && (calibration_data_.k4a_calibration_.depth_mode != K4A_DEPTH_MODE_PASSIVE_IR) &&
            (this->count_subscribers("Camera_" + index + "/rgb_to_depth/image_raw") > 0 || this->count_subscribers("Camera_" + index + "/rgb_to_depth/camera_info") > 0) &&
    ...
    if (this->count_subscribers("Camera_" + index + "/points2") > 0 &&
      (k4a_device_ || (capture.get_color_image() != nullptr && capture.get_depth_image() != nullptr)))
    {
      if (params_.rgb_point_cloud)
      {
  ...
  while (running_ && rclcpp::ok())
  {
    if (k4abt_tracker_queue_size_ > 0)
    {
      ...
      else
      {
        ...
        if (this->count_subscribers("Camera_" + index + "/body_tracking_data") > 0)
        {
          // Joint marker array
        ...
        if (this->count_subscribers("Camera_" + index + "/body_index_map/image_raw") > 0)
        {
          // Body index map
